...
verb 3" >>/etc/openvpn/client-template.txt
        ...
        chown nobody:$NOGROUP /etc/openvpn/clients
        chmod 700 /etc/openvpn/clients
        touch /var/log/openvpn-expiry.log
        chown nobody:$NOGROUP /var/log/openvpn-expiry.log
        chmod 644 /var/log/openvpn-expiry.log
        echo "#!/bin/bash
$(declare -f checkExpiredClients)
/etc/openvpn/easy-rsa/easyrsa --batch gen-crl
checkExpiredClients
systemctl restart openvpn-server@server
" >/usr/local/bin/check-openvpn-expiry.sh
        chmod +x /usr/local/bin/check-openvpn-expiry.sh
        echo "0 0 * * * /usr/local/bin/check-openvpn-expiry.sh >> /var/log/openvpn-expiry.log 2>&1" > /etc/cron.d/openvpn-expiry
...
function newClient() {
        ...
        echo ""
        echo "How many days should this VPN user be active?"
        until [[ $EXPIRY_DAYS =~ ^[0-9]+$ && $EXPIRY_DAYS -gt 0 ]]; do
                read -rp "Number of days (e.g., 30): " -e -i 30 EXPIRY_DAYS
        done
        EXPIRY_DATE=$(date -d "+$EXPIRY_DAYS days" +%s)
        echo "Client $CLIENT will expire on $(date -d "@$EXPIRY_DATE" '+%Y-%m-%d %H:%M:%S')."
        echo ""
        ...
        mkdir -p /etc/openvpn/clients
        echo "$EXPIRY_DATE" > "/etc/openvpn/clients/$CLIENT.expiry"
        mkdir -p /home/ubuntu
        ...
        echo ""
        echo "The configuration file has been written to /home/ubuntu/$CLIENT.ovpn."
        echo "Download the .ovpn file and import it in your OpenVPN client."
        echo "This client will be automatically revoked after $EXPIRY_DAYS days (on $(date -d "@$EXPIRY_DATE" '+%Y-%m-%d %H:%M:%S'))."
        exit 0
}
...
function revokeClient() {
        ...
        chmod 644 /etc/openvpn/crl.pem
        find /home/ubuntu -maxdepth 1 -name "$CLIENT.ovpn" -delete
        rm -f "/etc/openvpn/clients/$CLIENT.expiry"
...
function checkExpiredClients() {
        ...
        CURRENT_TIME=$(date +%s)
        for EXPIRY_FILE in "$CLIENT_DIR"/*.expiry; do
                if [[ -f $EXPIRY_FILE ]]; then
                        CLIENT=$(basename "$EXPIRY_FILE" .expiry)
                        EXPIRY_DATE=$(cat "$EXPIRY_FILE")
                        if [[ $CURRENT_TIME -ge $EXPIRY_DATE ]]; then
                                echo "Revoking expired client: $CLIENT"
                                cd /etc/openvpn/easy-rsa/ || return
                                ./easyrsa --batch revoke "$CLIENT"
                                EASYRSA_CRL_DAYS=3650 ./easyrsa gen-crl
                                rm -f /etc/openvpn/crl.pem
                                ...
                                sed -i "/^$CLIENT,.*/d" /etc/openvpn/ipp.txt
                                cp /etc/openvpn/easy-rsa/pki/index.txt{,.bk}
                                echo "Certificate for client $CLIENT revoked due to expiration."
                        fi
                fi
        done
}
...
function removeOpenVPN() {
        ...
        if [[ $REMOVE == 'y' ]]; then
                ...
                rm -f /usr/local/bin/check-openvpn-expiry.sh
                rm -f /etc/cron.d/openvpn-expiry
                rm -f /var/log/openvpn-expiry.log
